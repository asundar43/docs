...
read_when:
  - Designing workspace memory (~/.simpleclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep SimpleClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.simpleclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.simpleclaw/workspace` as canonical human-readable memory.
...
```
~/.simpleclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in SimpleClaw**, but keep a separable core library.

### Why integrate into SimpleClaw?

- SimpleClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `simpleclaw memory recall "…" --k 25 --since 30d`
  - `simpleclaw memory reflect --since 7d`
...
Pragmatic take for `~/.simpleclaw/workspace`:
