---
summary: "CLI reference for `simpleclaw qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
simpleclaw qr
simpleclaw qr --setup-code-only
simpleclaw qr --json
simpleclaw qr --remote
simpleclaw qr --url wss://gateway.example/ws --token '<token>'
```
