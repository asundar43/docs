...
- `/usage cost` in chats: local cost summary aggregated from SimpleClaw session logs.
- CLI: `simpleclaw status --usage` prints a full per-provider breakdown.
- CLI: `simpleclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
