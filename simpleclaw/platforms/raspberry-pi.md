---
summary: "SimpleClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SimpleClaw on a Raspberry Pi
  - Running SimpleClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# SimpleClaw on Raspberry Pi

## Goal

Run a persistent, always-on SimpleClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most SimpleClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled simpleclaw
...
```bash
# Check logs
journalctl -u simpleclaw --no-pager -n 100

# Common fix: rebuild
cd ~/simpleclaw  # if using hackable install
npm run build
sudo systemctl restart simpleclaw
```
