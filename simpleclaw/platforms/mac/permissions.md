...
- Same path: run the app from a fixed location (for SimpleClaw, `dist/SimpleClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.simpleclaw.mac
sudo tccutil reset ScreenCapture ai.simpleclaw.mac
...
Workaround: move files into the SimpleClaw workspace (`~/.simpleclaw/workspace`) if you want to avoid per-folder grants.
