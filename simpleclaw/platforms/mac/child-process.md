...
service via the external `simpleclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.simpleclaw.gateway`
  (or `ai.simpleclaw.<profile>` when using `--profile`/`SIMPLECLAW_PROFILE`; legacy `com.simpleclaw.*` is supported).
