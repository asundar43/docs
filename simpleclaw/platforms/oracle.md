---
summary: "SimpleClaw on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up SimpleClaw on Oracle Cloud
  - Looking for low-cost VPS hosting for SimpleClaw
  - Want 24/7 SimpleClaw on a small server
title: "Oracle Cloud"
---

# SimpleClaw on Oracle Cloud (OCI)

## Goal

Run a persistent SimpleClaw Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for SimpleClaw (especially if you already have an OCI account), but it comes with tradeoffs:
...
```bash
# Set hostname
sudo hostnamectl set-hostname simpleclaw
...
sudo tailscale up --ssh --hostname=simpleclaw
```

This enables Tailscale SSH, so you can connect via `ssh simpleclaw` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@simpleclaw` (or use the Tailscale IP).

## 5) Install SimpleClaw
...
simpleclaw config set gateway.bind loopback

# Require auth for the Gateway + Control UI
simpleclaw config set gateway.auth.mode token
simpleclaw doctor --generate-gateway-token

# Expose over Tailscale Serve (HTTPS + tailnet access)
simpleclaw config set gateway.tailscale.mode serve
simpleclaw config set gateway.trustedProxies '["127.0.0.1"]'
...
```
https://simpleclaw.<tailnet-name>.ts.net/
```
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `simpleclaw security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.simpleclaw`
- **Security audit:** `simpleclaw security audit`
...
ssh -L 18789:127.0.0.1:18789 ubuntu@simpleclaw
```
...
# Re-authenticate
sudo tailscale up --ssh --hostname=simpleclaw --reset
```
...
- `~/.simpleclaw/workspace/` — workspace (SOUL.md, memory, artifacts)
