...
```bash
simpleclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `simpleclaw tui --deliver`
...
- Check the Gateway logs: `simpleclaw logs --follow`.
- Confirm the agent can run: `simpleclaw status` and `simpleclaw models status`.
...
- No agents in picker: check `simpleclaw agents list` and your routing config.
