...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/simpleclaw`)
...
If the page fails to load, start the Gateway first: `simpleclaw gateway`.
...
you revoke it with `simpleclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.simpleclaw/simpleclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop simpleclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SimpleClaw
...
```bash
simpleclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
SimpleClaw **blocks** Control UI connections without device identity.
...
```bash
SIMPLECLAW_CONTROL_UI_BASE_PATH=/simpleclaw/ pnpm ui:build
```
