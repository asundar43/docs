...
- Jobs persist under `~/.simpleclaw/cron/` so restarts don’t lose schedules.
...
simpleclaw cron run <job-id>
simpleclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.simpleclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `simpleclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, SimpleClaw applies a
...
If `delivery` is omitted for isolated jobs, SimpleClaw defaults to `announce`.
...
- Job store: `~/.simpleclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.simpleclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
```json5
{
  cron: {
    ...
    store: "~/.simpleclaw/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, SimpleClaw archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `simpleclaw cron runs` and adjust retention before logs become large
...
simpleclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
simpleclaw cron edit <jobId> --agent ops
simpleclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
simpleclaw cron run <jobId>
simpleclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
simpleclaw cron edit <jobId> \
...
```bash
simpleclaw cron edit <jobId> --exact
```
...
```bash
simpleclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`simpleclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `SIMPLECLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- SimpleClaw applies exponential retry backoff for recurring jobs after consecutive errors:
