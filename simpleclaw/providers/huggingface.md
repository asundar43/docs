...
read_when:
  - You want to use Hugging Face Inference with SimpleClaw
...
[Hugging Face Inference Providers](https://huggingface.co/docs/inference-providers) offer OpenAI-compatible chat completions through a single router API. You get access to many models (DeepSeek, Llama, and more) with one token. SimpleClaw uses the **OpenAI-compatible endpoint** (chat completions only); for text-to-image, embeddings, or speech use the [HF inference clients](https://huggingface.co/docs/api-inference/quicktour) directly.
...
```bash
simpleclaw onboard --auth-choice huggingface-api-key
```
...
is available to that process (for example, in `~/.simpleclaw/.env` or via
`env.shellEnv`).

## Model discovery and onboarding dropdown

SimpleClaw discovers models by calling the **Inference endpoint directly**:
