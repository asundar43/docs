---
summary: "Use MiniMax M2.1 in SimpleClaw"
read_when:
  - You want MiniMax models in SimpleClaw
...
```bash
simpleclaw plugins enable minimax-portal-auth  # skip if already loaded.
simpleclaw gateway restart  # restart if gateway is already running
simpleclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/simpleclaw/simpleclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `simpleclaw.json`:
...
- Use `simpleclaw models list` and `simpleclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `simpleclaw configure` and selecting **MiniMax M2.1**, or
