---
summary: "Use Synthetic's Anthropic-compatible API in SimpleClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. SimpleClaw registers it as the
...
```bash
simpleclaw onboard --auth-choice synthetic-api-key
```
...
Note: SimpleClaw's Anthropic client appends `/v1` to the base URL, so use
