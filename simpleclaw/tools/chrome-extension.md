---
summary: "Chrome extension: let SimpleClaw drive your existing Chrome tab"
read_when:
...
The SimpleClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate simpleclaw-managed Chrome profile.
...
SimpleClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the SimpleClaw release (npm package) as static files. There is no separate “build” step.

After upgrading SimpleClaw:

- Re-run `simpleclaw browser extension install` to refresh the installed files under your SimpleClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

SimpleClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `SIMPLECLAW_GATEWAY_TOKEN`)
...
- Open the tab you want SimpleClaw to control.
...
- `ON`: attached; SimpleClaw can drive that tab.
...
Debugging: `simpleclaw sandbox explain`
...
`simpleclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `simpleclaw browser extension install` first to copy the extension to a stable location under your SimpleClaw state directory.
...
- **This is not isolated** like the dedicated simpleclaw-managed profile.
