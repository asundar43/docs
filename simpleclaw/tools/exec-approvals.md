...
- **gateway host** → `simpleclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.simpleclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.simpleclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `simpleclaw approvals allowlist ...`).
- `simpleclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `simpleclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.simpleclaw/exec-approvals.json` directly.

CLI: `simpleclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
