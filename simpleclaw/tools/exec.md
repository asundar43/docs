...
- `gateway`/`node` approvals are controlled by `~/.simpleclaw/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  SimpleClaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`simpleclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `simpleclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
