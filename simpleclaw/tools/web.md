...
SimpleClaw ships two lightweight web tools:
...
If no `provider` is explicitly set, SimpleClaw auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `simpleclaw configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `simpleclaw configure --section web`. It stores the key in
`~/.simpleclaw/simpleclaw.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.simpleclaw/.env` (or your
service environment). See [Env vars](/help/faq#how-does-simpleclaw-load-environment-variables).
...
environment. For a gateway install, put it in `~/.simpleclaw/.env`.

If no base URL is set, SimpleClaw chooses a default based on the API key source:
...
For a gateway install, put it in `~/.simpleclaw/.env`.
