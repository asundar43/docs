...
description: "Complete field-by-field reference for ~/.simpleclaw/simpleclaw.json"
summary: "Complete reference for every SimpleClaw config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.simpleclaw/simpleclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — SimpleClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `simpleclaw doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["simpleclaw-dm"] },
      guilds: {
        "123456789012345678": {
...
- SimpleClaw additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `simpleclaw plugins install @simpleclaw/mattermost`.
...
SimpleClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@simpleclaw", "simpleclaw"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@simpleclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `simpleclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, SimpleClaw auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/simpleclaw" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, SimpleClaw assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "simpleclaw-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "simpleclaw-sandbox-browser:bookworm-slim",
          network: "simpleclaw-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.simpleclaw/sandboxes`
...
noVNC observer access uses VNC auth by default and SimpleClaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `simpleclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.simpleclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@simpleclaw"] },
        sandbox: { mode: "off" },
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.simpleclaw/workspace-home" },
      { id: "work", workspace: "~/.simpleclaw/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.simpleclaw/settings/tts.json",
      elevenlabs: {
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:simpleclaw`   | All built-in tools (excludes provider plugins)                                           |
...
SimpleClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.simpleclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SIMPLECLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `simpleclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `simpleclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `simpleclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `simpleclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `simpleclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `simpleclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.simpleclaw/extensions`, `<workspace>/.simpleclaw/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SIMPLECLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SIMPLECLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
```bash
SIMPLECLAW_CONFIG_PATH=~/.simpleclaw/a.json \
SIMPLECLAW_STATE_DIR=~/.simpleclaw-a \
simpleclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.simpleclaw-dev` + port `19001`), `--profile <name>` (uses `~/.simpleclaw-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-simpleclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SIMPLECLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.simpleclaw/workspace/canvas",
    liveReload: true,
    // enabled: false, // or SIMPLECLAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__simpleclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__simpleclaw__/a2ui/`
...
- Also serves A2UI at `/__simpleclaw__/a2ui/`.
...
- Hostname defaults to `simpleclaw`. Override with `SIMPLECLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.simpleclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `simpleclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.simpleclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SIMPLECLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.simpleclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/simpleclaw/simpleclaw-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `simpleclaw doctor --fix` can strip unknown keys).
