...
`simpleclaw doctor` is the repair + migration tool for SimpleClaw. It fixes stale
...
```bash
simpleclaw doctor --repair --force
```
...
```bash
cat ~/.simpleclaw/simpleclaw.json
```
...
- Extra workspace dir detection (`~/simpleclaw`).
...
you to run `simpleclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.simpleclaw/simpleclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.simpleclaw/sessions/` to `~/.simpleclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.simpleclaw/agent/` to `~/.simpleclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.simpleclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.simpleclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `simpleclaw doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.simpleclaw` folders exist across
  home directories or when `SIMPLECLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.simpleclaw/simpleclaw.json` is
...
offers to remove them and install the SimpleClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named SimpleClaw gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `simpleclaw doctor --generate-gateway-token` to force token
...
- `simpleclaw doctor` prompts before rewriting supervisor config.
- `simpleclaw doctor --yes` accepts the default repair prompts.
- `simpleclaw doctor --repair` applies recommended fixes without prompts.
- `simpleclaw doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `simpleclaw gateway install --force`.
