...
- During heartbeat runs, SimpleClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `simpleclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, SimpleClaw skips the heartbeat run entirely (no model call).
...
```bash
simpleclaw system event --text "Check for urgent follow-ups" --mode now
```
