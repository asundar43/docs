...
read_when:
  - Running SimpleClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of SimpleClaw
...
- You run SimpleClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. SimpleClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. SimpleClaw extracts the user identity from the configured header
...
- SimpleClaw can stay on loopback HTTP behind the proxy.
...
If SimpleClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://simpleclaw.example.com
    to: http://simpleclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
simpleclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy simpleclaw:18789 {
...
location / {
    ...
    proxy_pass http://simpleclaw:18789;
...
`simpleclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update SimpleClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `simpleclaw security audit` and review findings
