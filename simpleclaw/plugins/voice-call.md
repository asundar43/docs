...
read_when:
  - You want to place an outbound voice call from SimpleClaw
...
Voice calls for SimpleClaw via a plugin. Supports outbound notifications and
...
- Use `simpleclaw voicecall ...` or the `voice_call` tool
...
```bash
simpleclaw plugins install ./extensions/voice-call
...
```bash
simpleclaw voicecall call --to "+15555550123" --message "Hello from SimpleClaw"
simpleclaw voicecall continue --call-id <id> --message "Any questions?"
simpleclaw voicecall speak --call-id <id> --message "One moment"
simpleclaw voicecall end --call-id <id>
simpleclaw voicecall status --call-id <id>
simpleclaw voicecall tail
