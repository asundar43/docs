...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SimpleClaw
...
Running SimpleClaw via Node with `tsx` fails at startup with:

```
[simpleclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `simpleclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch simpleclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node simpleclaw.mjs status` works on Node 25.
