...
- Publish as `simpleclaw/plugin-sdk` (or export from core under `simpleclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `SimpleClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: SimpleClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: SimpleClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: SimpleClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: SimpleClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: SimpleClawConfig): string;
  };
};
```
...
- Introduce `simpleclaw/plugin-sdk`.
- Add `api.runtime` to `SimpleClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `simpleclawRuntime: ">=2026.2.0"`).
