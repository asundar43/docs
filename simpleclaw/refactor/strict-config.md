...
- Plugin manifests (`simpleclaw.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`simpleclaw.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `simpleclaw doctor --fix`.
- `simpleclaw doctor --fix`:
  - Applies migrations.
...
Everything else must hard-fail with: “Config invalid. Run `simpleclaw doctor --fix`.”
