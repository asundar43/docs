...
read_when:
  - Setting up Twitch chat integration for SimpleClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. SimpleClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
simpleclaw plugins install @simpleclaw/twitch
```

Local checkout (when running from a git repo):

```bash
simpleclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `SIMPLECLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "simpleclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SIMPLECLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
SIMPLECLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
