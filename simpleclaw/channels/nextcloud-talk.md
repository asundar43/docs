...
```bash
simpleclaw plugins install @simpleclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
simpleclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SimpleClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `simpleclaw pairing list nextcloud-talk`
  - `simpleclaw pairing approve nextcloud-talk <CODE>`
