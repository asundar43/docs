...
Matrix is an open, decentralized messaging protocol. SimpleClaw connects as a Matrix **user**
...
```bash
simpleclaw plugins install @simpleclaw/matrix
```

Local checkout (when running from a git repo):

```bash
simpleclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `simpleclaw plugins install @simpleclaw/matrix`
   - From a local checkout: `simpleclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: SimpleClaw calls the same
     login endpoint, stores the access token in `~/.simpleclaw/credentials/matrix/credentials.json`,
...
- On first connection, SimpleClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  SimpleClaw logs a warning.
...
`~/.simpleclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `simpleclaw pairing list matrix`
  - `simpleclaw pairing approve matrix <CODE>`
...
- On startup, SimpleClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
