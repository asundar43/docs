---
summary: "Mattermost bot setup and SimpleClaw config"
read_when:
...
```bash
simpleclaw plugins install @simpleclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
simpleclaw plugins install ./extensions/mattermost
```
...
4. Configure SimpleClaw and start the gateway.
...
- Approve via:
  - `simpleclaw pairing list mattermost`
  - `simpleclaw pairing approve mattermost <CODE>`
...
Use these target formats with `simpleclaw message send` or cron/webhooks:
