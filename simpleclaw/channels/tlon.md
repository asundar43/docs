...
Tlon is a decentralized messenger built on Urbit. SimpleClaw connects to your Urbit ship and can
...
```bash
simpleclaw plugins install @simpleclaw/tlon
```

Local checkout (when running from a git repo):

```bash
simpleclaw plugins install ./extensions/tlon
```
...
By default, SimpleClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `simpleclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SimpleClaw replies in-thread.
