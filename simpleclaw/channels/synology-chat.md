---
summary: "Synology Chat webhook setup and SimpleClaw config"
read_when:
  - Setting up Synology Chat with SimpleClaw
...
```bash
simpleclaw plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your SimpleClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in SimpleClaw.
...
- Pairing approvals work with:
  - `simpleclaw pairing list synology-chat`
  - `simpleclaw pairing approve synology-chat <CODE>`
...
```bash
simpleclaw message send --channel synology-chat --target 123456 --text "Hello from SimpleClaw"
simpleclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
