---
summary: "How SimpleClaw builds prompt context and reports token usage + costs"
read_when:
...
SimpleClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

SimpleClaw assembles its own system prompt on every run. It includes:
...
For images, SimpleClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from SimpleClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `simpleclaw status --usage` and `simpleclaw channels list` show
...
`cacheWrite`. If pricing is missing, SimpleClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. SimpleClaw can
...
Anthropic's 1M context window is currently beta-gated. SimpleClaw can inject the
...
SimpleClaw skips the `context-1m-*` beta header because Anthropic currently
