...
SimpleClaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SimpleClaw owns lifecycle when `channels.signal.autoStart=true`.
...
- SimpleClaw spawns `imsg rpc` as a child process (legacy iMessage integration).
