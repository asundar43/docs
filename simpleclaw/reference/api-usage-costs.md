...
SimpleClaw features that can generate provider usage or paid API calls.
...
- `simpleclaw status --usage` and `simpleclaw channels list` show provider **usage windows**
...
SimpleClaw can pick up credentials from:
...
- `simpleclaw status --usage`
- `simpleclaw models status --json`
...
`simpleclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
