...
- With Anthropic API-key auth profiles, SimpleClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, SimpleClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
SimpleClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.simpleclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$SIMPLECLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `SIMPLECLAW_CACHE_TRACE=1` enables cache tracing.
- `SIMPLECLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `SIMPLECLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `SIMPLECLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `SIMPLECLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
